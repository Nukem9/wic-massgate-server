...
MMG_ServerFilter::MMG_ServerFilter(ushort activefilters)
{
	memset(this->partOfServerName, 0, sizeof(this->partOfServerName));
	memset(this->partOfMapName, 0, sizeof(this->partOfMapName));
	this->requiresPassword		= false;
...
{
	if (this->HasFlag(DEDICATED_FLAG) && !aMessage->ReadBool(this->isDedicated))
		return false;

	if (this->HasFlag(RANKED_FLAG) && !aMessage->ReadBool(this->isRanked))
		return false;

	if (this->HasFlag(RANKBALANCE_FLAG) && !aMessage->ReadBool(this->isRankBalanced))
		return false;

	if (this->HasFlag(GAMEMODE_FLAG) && !aMessage->ReadBool(this->hasDominationMaps))
		return false;

	if (this->HasFlag(GAMEMODE_FLAG) && !aMessage->ReadBool(this->hasAssaultMaps))
		return false;

	if (this->HasFlag(GAMEMODE_FLAG) && !aMessage->ReadBool(this->hasTowMaps))
		return false;

	if (this->HasFlag(TOTALSLOTS_FLAG) && !aMessage->ReadUChar(this->totalSlots))
		return false;

	if (this->HasFlag(EMPTYSLOTS_FLAG) && !aMessage->ReadUChar(this->emptySlots))
		return false;

	// always reads population, massgate crashes otherwise
	if (this->HasFlag(NOTFULLEMPTY_FLAG) && !aMessage->ReadUChar(this->notFullEmpty))
		return false;

	if (this->HasFlag(PLAYNOW_FLAG) && !aMessage->ReadBool(this->fromPlayNow))
		return false;
	...
	if (this->HasFlag(PASSWORD_FLAG) && !aMessage->ReadBool(this->requiresPassword))
		return false;

	if (this->HasFlag(MOD_FLAG) && !aMessage->ReadBool(this->noMod))
		return false;
...
void MMG_ServerFilter::PrintFilters()
{
	...
	if (this->HasFlag(GAMEMODE_FLAG) && this->hasDominationMaps)
		DebugLog(L_INFO, "gamemode flag set\t\t: Domination Only");

	if (this->HasFlag(GAMEMODE_FLAG) && this->hasAssaultMaps)
		DebugLog(L_INFO, "gamemode flag set\t\t: Assault Only");

	if (this->HasFlag(GAMEMODE_FLAG) && this->hasTowMaps)
